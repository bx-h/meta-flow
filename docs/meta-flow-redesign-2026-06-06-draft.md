...
> 注：这是早期重构设计草稿。`0.1.8` 后当前实现优先使用 `meta-flow ...` / `metaflow ...` CLI，runtime task state 默认位于 `~/.meta-flow`，by-node artifact path 同时作为人类视图和机器契约，并在 QUESTIONING 阶段更倾向通过 clarification gate 询问用户。
